    ...
    <header className="sticky top-0 z-50 bg-navy text-white border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[68px] flex items-center justify-between">
        <div
          ...
          className="flex items-center space-x-2.5 sm:space-x-3 cursor-pointer group min-w-0 mr-2"
        >
        ...
        <div className="hidden lg:flex items-center space-x-3">
          ...
          {isAuthenticated ? (
            <>
              <button
                onClick={() => onNavigate(isStaff ? 'admin_case_list' : 'client_dashboard')}
                className="text-xs px-2.5 py-1.5 rounded-md border border-white/15 bg-white/5 text-white/90 hover:bg-white/10 flex items-center space-x-1.5 transition"
                title="Panelime git"
              >
        ...
        <div className="lg:hidden flex items-center space-x-2 shrink-0">
          <div className="flex items-center bg-white/5 p-1 rounded border border-white/10 text-xs">
            {(['TR', 'PL', 'EN'] as Language[]).map(lang => (
              <button
                key={lang}
                onClick={() => onLanguageChange(lang)}
                className={`px-1.5 py-0.5 rounded ${
                  currentLanguage === lang ? 'bg-gold text-navy font-bold' : 'text-white/65'
                }`}
              >
                {lang}
              </button>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-navy-2 border-t border-white/10 px-4 pt-3 pb-6 space-y-3">
          <div className="font-semibold text-xs text-white/45 uppercase tracking-wider mb-1">
            Menü
          </div>
          <div className="grid grid-cols-1 gap-1">
            ...
            {isClient && (
              <button
                ...
                className={`text-left px-3 py-2 rounded-md text-sm transition ${
                  activeScreen === 'client_dashboard' ? 'bg-gold text-navy font-bold' : 'text-white/80 hover:bg-white/5'
                }`}
              >
                {t.nav_dashboard}
              </button>
            ...
            {isStaff && (
              <button
                ...
                className={`text-left px-3 py-2 rounded-md text-sm transition ${
                  activeScreen.startsWith('admin') ? 'bg-gold text-navy font-bold' : 'text-white/80 hover:bg-white/5'
                }`}
              >
                {t.nav_admin}
              </button>
